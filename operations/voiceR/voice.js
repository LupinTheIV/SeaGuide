...
document.querySelector("#voz").addEventListener('click', function(){
    if(!act){
        ...
        artyom.say("Activando el servicio de reconocimiento de voz");
        document.getElementById("voz").style.color = "darkgreen";
        act = !act;
    }else{
        artyom.say("Desactivando el servicio de reconocimiento de voz");
        document.getElementById("voz").style.color = "black";
        artyom.fatality().then(() => {
            console.log("Artyom off")
        });
        act = !act;
    }
...
artyom.addCommands({
    ...
    action: function(i){
        if(i == 0){      
            ...
            document.querySelector("#mainRoomDoor").style.transform = "rotate(20deg)"//REVISAR
        }else if(i == 15){
